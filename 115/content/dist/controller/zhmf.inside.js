    ...
    exports('zhmf', {
        ...
        ajax: function(params) {
            ...
            if (!zhmf.access_token && params.login) {
                //删除用户相关的所有信息，跳登录，重新获取信息
                this.store.delete(setter.tableName);
                return location.hash = search.redirect ? decodeURIComponent(search.redirect) : '/user/login';
            } else {
                //登录状态向后顺延2小时
                this.store.set(setter.tableName,zhmf,7200);
        ...
        //store缓存机制
        //@set(table, info, expires),写入缓存信息，table(字符串),存储到缓存中的表名，info(对象),存储到table中的数据对象，expires(数字)存储到table中的数据再过多长时间过期，单位秒
