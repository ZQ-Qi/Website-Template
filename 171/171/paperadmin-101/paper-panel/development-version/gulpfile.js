...
gulp.task('styles', function () {
    return gulp.src([paths.srcCSS + 'app.scss'])
        .pipe(named())
...
gulp.task('watch', gulp.series('default', function () {
    ...
    gulp.watch('app/src/*.html').on('change', function (file) {
        ...
        return gulp.src('app/src/'+leafname)
            .pipe(injectPartials({
                removeTags: true,
                ignoreError: true
            }))
...
gulp.task('production-min-tf', gulp.series(
    'clean:dist',
    'html:dist',
    'css:dist-min',
    'js:dist-min',
...
gulp.task('tf-version', gulp.series(['clean:tf', 'production-min-tf', 'placeholder', 'previews-resize', 'img-compress'], function () {
