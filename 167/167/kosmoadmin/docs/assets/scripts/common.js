...
$.LoadingOverlaySetup({
    image: "libs/loading-overlay/loading.gif",
    /*image       : "",
...
window.Kosmo = {
    ...
    initSidebarScrollBar: function(ksSidebar, isSidebarFixed) {
        if (isSidebarFixed) {
            ksSidebar.find('> .ks-wrapper').jScrollPane({
...
$(function() {
    ...
    /*$('.ks-content').css({
        marginTop: -($('.ks-page > .ks-header').height())
    });
    $('.ks-content > .ks-body').css({
        paddingTop: $('.ks-page > .ks-header').height()
    });*/
    ...
    $('[data-auto-height]').each(function() {
        ...
        var fixHeight = parseInt(self.data('fix-height'));
        ...
        if (height <= 0) {
            if (self.data('min-height')) {
                height = parseInt(self.data('min-height'));
            } else {
                height = 200;
            }
        }
    ...
    if ($('.ks-navbar-horizontal').size()) {
        ...
        if (ksNavbarHorizontalScrollWidth > ksNavbarHorizontalWidth) {
            ...
            $($('.ks-navbar-horizontal > .nav > li:not(.ks-navbar-horizontal-responsive)').get().reverse()).each(function(index, element) {
